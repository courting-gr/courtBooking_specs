...
- [x] 1. Domain layer — Notification and DeviceToken entities, enums, value objects (Transaction Service)
  - [x] 1.1 Create `NotificationType` enum with all 28 notification types
    - File: `domain/model/NotificationType.java`
    - _Requirements: 1.1, 11_
  - [x] 1.2 Create `UrgencyLevel` enum (CRITICAL, STANDARD, PROMOTIONAL)
    - File: `domain/model/UrgencyLevel.java`
    - _Requirements: 1.4, 1.5, 1.6_
  - [x] 1.3 Create `TokenType` enum (FCM, WEB_PUSH) and `ChannelStatus` enum (PENDING, DELIVERED, FAILED, SKIPPED)
    - Files: `domain/model/TokenType.java`, `domain/model/ChannelStatus.java`
    - _Requirements: 2.5, 9.3, 12.5_
  - [x] 1.4 Create `DeliveryStatus` value object with per-channel status tracking and overall status derivation
  ...
  - [x] 1.5 Create `Notification` rich domain entity
  ...
  - [x] 1.6 Create `DeviceToken` rich domain entity
  ...
  - [x] 1.7 Create `NotificationEvent` internal record/DTO for Kafka consumer deserialization
...
- [x] 2. Database migrations (Transaction Service V3–V5, Platform Service V7)
  - [x] 2.1 Create V3 migration — Restructure notifications table for Phase 5
  ...
  - [x] 2.2 Create V4 migration — Restructure device_tokens table for Phase 5
  ...
  - [x] 2.3 Create V5 migration — Add reminder_state JSONB column to bookings table
    ...
    - _Requirements: 16.3_
  - [x] 2.4 Create V7 migration in Platform Service — Add DND columns to court_owner_notif_prefs
...
- [x] 3. Outgoing port interfaces (Transaction Service)
  - [x] 3.1 Create `FcmPushPort` interface with `FcmMessage`, `FcmDeliveryResult`, `TokenResult` records
    - File: `application/port/out/FcmPushPort.java`
    - _Requirements: 3.1, 3.2_
  - [x] 3.2 Create `SendGridEmailPort` interface with `SendGridEmailRequest` record
    - File: `application/port/out/SendGridEmailPort.java`
    - _Requirements: 8.1_
  - [x] 3.3 Create `WebPushPort` interface with `WebPushSubscription`, `WebPushPayload`, `WebPushDeliveryResult` records
    - File: `application/port/out/WebPushPort.java`
    - _Requirements: 9.4_
  - [x] 3.4 Create `WebSocketSessionPort` interface (isUserConnected, getSessionIds, sendToUser)
    - File: `application/port/out/WebSocketSessionPort.java`
    - _Requirements: 4.10, 7.1_
  - [x] 3.5 Create `RedisPubSubPublisherPort` interface (publishNotification, publishAvailabilityUpdate, publishBookingUpdate)
    - File: `application/port/out/RedisPubSubPublisherPort.java`
    - _Requirements: 5.1, 5.2_
  - [x] 3.6 Create `SaveNotificationPort` and `LoadNotificationPort` interfaces
  ...
  - [x] 3.7 Create `SaveDeviceTokenPort` and `LoadDeviceTokenPort` interfaces
  ...
  - [x] 3.8 Create `NotificationDeduplicationPort` interface (isProcessed, markProcessed)
    - File: `application/port/out/NotificationDeduplicationPort.java`
    - _Requirements: 1.11, 22.1_
  - [x] 3.9 Create `NotificationPreferenceCachePort` interface with `UserNotificationPreferences` record (including nested `ChannelPreference` and `DndConfiguration`)
    ...
    - _Requirements: 20.5_
  - [x] 3.10 Extend existing `PlatformServicePort` with Phase 5 methods: `getNotificationPreferences`, `getReminderRules`, `updateDnd`, and `ReminderRuleInfo` record
    - File: modify existing `application/port/out/PlatformServicePort.java`
    - _Requirements: 20.1, 20.2, 20.7_

- [x] 4. Outgoing port interface — Platform Service (new)
  - [x] 4.1 Create `AvailabilityPubSubPort` interface with `AvailabilityUpdateMessage` record
    - File: `platform-service/.../application/port/out/AvailabilityPubSubPort.java`
    - _Requirements: 6.1, 28.1_

- [x] 5. Incoming port interfaces (Use Cases) — Transaction Service
  - [x] 5.1 Create `ProcessNotificationUseCase` interface
    ...
    - _Requirements: 1.1_
  - [x] 5.2 Create device token use cases: `RegisterDeviceTokenUseCase`, `RemoveDeviceTokenUseCase`, `ListDeviceTokensQuery` with `RegisterDeviceCommand` and `DeviceTokenResult` records
  ...
  - [x] 5.3 Create Web Push use cases: `GetVapidKeyQuery`, `SubscribeWebPushUseCase`, `UnsubscribeWebPushUseCase` with `SubscribeWebPushCommand` and `WebPushSubscriptionResult` records
  ...
  - [x] 5.4 Create notification center use cases: `ListNotificationsQuery`, `MarkNotificationReadUseCase`, `MarkAllNotificationsReadUseCase`, `GetUnreadCountQuery` with `NotificationFilter` and `NotificationResult` records
  ...
  - [x] 5.5 Create `UpdateDndConfigurationUseCase` with `UpdateDndCommand` and `DndConfigurationResult` records
    - File: `application/port/in/UpdateDndConfigurationUseCase.java`
    - _Requirements: 10.6_
  - [x] 5.6 Create `ScheduleBookingReminderUseCase` and `CancelBookingReminderUseCase` interfaces
    - Files: `application/port/in/ScheduleBookingReminderUseCase.java`, `application/port/in/CancelBookingReminderUseCase.java`
    - _Requirements: 15.1, 15.3_
  - [x] 5.7 Create WebSocket notification and availability update DTO/record classes
...
- [x] 6. Checkpoint — Ensure all domain, port, and migration code compiles
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Shared retry utility, AES-256-GCM encryption, and SendGrid template resolver (Transaction Service)
  - [x] 7.1 Create `RetryWithBackoff` utility component
  ...
  - [x] 7.2 Create `DeviceTokenEncryptor` component
  ...
  - [x] 7.4 Create `SendGridTemplateResolver` component
...
- [x] 8. Persistence adapters — Notification and DeviceToken (Transaction Service)
  - [x] 8.1 Create `NotificationJpaEntity` with JPA mappings for the restructured notifications table
    ...
    - _Requirements: 14.1_
  - [x] 8.2 Create `NotificationRepository` (Spring Data JPA) with custom queries for DND-queued, unread count, mark-all-read, delete-older-than, count-by-user, delete-oldest-read, **loadPendingWebSocket** (notifications with `webSocketStatus = PENDING` for reconnection delivery per Req 13.3)
    - File: `adapter/out/persistence/repository/NotificationRepository.java`
    - _Requirements: 7.5, 7.8, 10.7, 13.3, 14.2, 14.5_
  - [x] 8.3 Create `NotificationPersistenceAdapter` implementing `SaveNotificationPort` and `LoadNotificationPort`
  ...
  - [x] 8.4 Create `DeviceTokenJpaEntity` with JPA mappings for the restructured device_tokens table
    - File: `adapter/out/persistence/entity/DeviceTokenJpaEntity.java`
    - _Requirements: 2.5_
  - [x] 8.5 Create `DeviceTokenRepository` (Spring Data JPA) with queries for user tokens by type, by encrypted token, expired token cleanup
    - File: `adapter/out/persistence/repository/DeviceTokenRepository.java`
    - _Requirements: 2.1, 2.2, 2.6, 18.1_
  - [x] 8.6 Create `DeviceTokenPersistenceAdapter` implementing `SaveDeviceTokenPort` and `LoadDeviceTokenPort`
  ...
  - [x] 8.7 Update `BookingJpaEntity` — add `reminderState` JSONB field with `ReminderStateConverter`
    ...
    - _Requirements: 16.3_
  - [x] 8.8 Update `Booking` domain model — add `reminderState` field and methods (`hasReminderBeenSent`, `markReminderSent`)
    ...
    - _Requirements: 16.3_
  - [x] 8.9 Update `BookingPersistenceMapper` to map `reminderState` between entity and domain
    - Modify existing `adapter/out/persistence/mapper/BookingPersistenceMapper.java`
    - _Requirements: 16.3_
  - [x] 8.10 Update existing tests and `IntegrationTestConfiguration` for the new `reminderState` field
...
- [x] 9. Redis adapters — Deduplication and Preference Cache (Transaction Service)
  - [x] 9.1 Create `NotificationDeduplicationRedisAdapter` implementing `NotificationDeduplicationPort`
  ...
  - [x] 9.2 Create `NotificationPreferenceCacheRedisAdapter` implementing `NotificationPreferenceCachePort`
...
- [x] 10. External service adapters — FCM, SendGrid, Web Push (Transaction Service)
  - [x] 10.1 Create `FcmPushAdapter` implementing `FcmPushPort`
  ...
  - [x] 10.2 Create `NoOpFcmPushAdapter` with `@ConditionalOnMissingBean(FirebaseApp.class)`
  ...
  - [x] 10.3 Create `FirebaseConfig` — initializes `FirebaseApp` from credentials path
  ...
  - [x] 10.5 Create `SendGridEmailAdapter` implementing `SendGridEmailPort`
  ...
  - [x] 10.6 Create `NoOpSendGridEmailAdapter` with `@ConditionalOnProperty(name = "sendgrid.api-key", havingValue = "SG.placeholder")`
    - File: `adapter/out/email/NoOpSendGridEmailAdapter.java`
    - _Requirements: Design — NoOp pattern_
  - [x] 10.7 Create `WebPushAdapter` implementing `WebPushPort`
  ...
  - [x] 10.8 Create `NoOpWebPushAdapter` with `@ConditionalOnProperty(name = "web-push.vapid-private-key", havingValue = "")`
    - File: `adapter/out/webpush/NoOpWebPushAdapter.java`
    - _Requirements: Design — NoOp pattern_
  - [x] 10.9 Create `WebPushConfig` — initializes web-push library with VAPID keys
...
- [x] 11. Checkpoint — Ensure all adapters compile and NoOp adapters are wired
  - Ensure all tests pass, ask the user if questions arise.

- [x] 12. Redis Pub/Sub publisher adapter (Transaction Service)
  - [x] 12.1 Create `RedisPubSubPublisherAdapter` implementing `RedisPubSubPublisherPort`
  ...
  - [x] 12.2 Create `NoOpRedisPubSubPublisher` for tests/environments without Redis
    - File: `adapter/out/redis/NoOpRedisPubSubPublisher.java`
    - _Requirements: Design — NoOp pattern_

- [x] 13. PlatformServiceHttpClient extension (Transaction Service)
  - [x] 13.1 Extend `PlatformServiceHttpClient` with `getNotificationPreferences(UserId)` method
    - Calls `GET /internal/users/{userId}/notification-preferences`, deserializes to `UserNotificationPreferences`
    - _Requirements: 20.1, 20.5_
  - [x] 13.2 Extend `PlatformServiceHttpClient` with `getReminderRules(CourtId)` method
    - Calls `GET /internal/courts/{courtId}/reminder-rules`, deserializes to `List<ReminderRuleInfo>`
    - _Requirements: 20.2_
  - [x] 13.3 Extend `PlatformServiceHttpClient` with `updateDnd(UserId, ...)` method
    - Calls `PUT /internal/users/{userId}/dnd`
    - _Requirements: 20.7_

- [x] 14. Application services — Device Token, Notification Center, DND (Transaction Service)
  - [x] 14.1 Create `DeviceTokenService` implementing `RegisterDeviceTokenUseCase`, `RemoveDeviceTokenUseCase`, `ListDeviceTokensQuery`
  ...
  - [x] 14.4 Create `NotificationQueryService` implementing `ListNotificationsQuery`, `MarkNotificationReadUseCase`, `MarkAllNotificationsReadUseCase`, `GetUnreadCountQuery`
  ...
  - [x] 14.6 Create `DndConfigurationService` implementing `UpdateDndConfigurationUseCase`
    ...
    - _Requirements: 10.6_
  - [x] 14.7 Create `WebPushSubscriptionService` implementing `GetVapidKeyQuery`, `SubscribeWebPushUseCase`, `UnsubscribeWebPushUseCase`
...
- [x] 15. WebSocket infrastructure (Transaction Service)
  - [x] 15.1 Create `WebSocketConfig` — STOMP-over-WebSocket configuration
  ...
  - [x] 15.2 Create `JwtHandshakeInterceptor` — validates JWT from query param on WebSocket handshake
  ...
  - [x] 15.3 Create `StompSubscriptionInterceptor` — authorizes STOMP subscriptions
  ...
  - [x] 15.4 Create `WebSocketConnectionRegistry` — in-memory concurrent map of userId → Set<sessionId>
    ...
    - _Requirements: 4.10_
  - [x] 15.5 Create `WebSocketEventListener` — handles STOMP connect/disconnect events
  ...
  - [x] 15.6 Create `WebSocketSessionAdapter` implementing `WebSocketSessionPort`
  ...
  - [x] 15.7 Create `WebSocketGracefulShutdown` — @PreDestroy lifecycle hook
  ...
  - [x] 15.8 Create `WebSocketThreadPoolConfig` — dedicated thread pool for WebSocket broadcasting
    - File: `config/WebSocketThreadPoolConfig.java`
    - Core 4, max 8, queue 1000, CallerRunsPolicy
    - _Requirements: 19.4_
  - [x] 15.9 Update `SecurityConfig` — add `/ws/**` to permitAll (WebSocket auth handled by interceptor)
    - Modify existing `config/SecurityConfig.java`
    - _Requirements: 4.1, 27_

- [x] 16. Redis Pub/Sub listener and broadcasting (Transaction Service)
  - [x] 16.1 Create `RedisPubSubConfig` — configure `RedisMessageListenerContainer` with WebSocket broadcast listeners and auto-reconnection
  ...
  - [x] 16.2 Create `WebSocketBroadcastListener` — receives Redis Pub/Sub messages and broadcasts to local WebSocket clients
...
- [x] 17. Checkpoint — Ensure WebSocket infrastructure compiles and Redis Pub/Sub is wired
  - Ensure all tests pass, ask the user if questions arise.

- [x] 18. Notification Processor pipeline (Transaction Service)
  ...
  - [x] 18.2 Create `NotificationProcessingService` implementing `ProcessNotificationUseCase`
  ...
  - [x] 18.12 Create `NotificationEventKafkaConsumer` — Kafka listener for `notification-events` topic
...
- [x] 19. REST controllers — Notification APIs (Transaction Service)
  - [x] 19.1 Create `DeviceTokenController`
    ...
    - _Requirements: 2.1, 2.3, 2.4_
  - [x] 19.2 Create `NotificationController`
    ...
    - _Requirements: 7.5, 7.6, 7.7, 7.8, 24.1, 24.2, 24.3_
  - [x] 19.3 Create `WebPushController`
...
- [x] 20. Checkpoint — Ensure Notification Processor and REST controllers compile
  - Ensure all tests pass, ask the user if questions arise.

- [x] 21. Platform Service changes — Internal APIs, BookingEventKafkaConsumer extension, AvailabilityPubSub
  - [x] 21.1 Create `DndConfiguration` value object in Platform Service domain
    ...
    - _Requirements: 31.1_
  - [x] 21.2 Extend `NotificationPreferencesPersistencePort` (or create `DndConfigurationPersistencePort`) for DND column access
  ...
  - [x] 21.3 Create or extend use case for fetching notification preferences + DND for internal API
  ...
  - [x] 21.4 Create or extend use case for updating DND configuration
  ...
  - [x] 21.5 Extend `InternalUserController` — add `GET /internal/users/{userId}/notification-preferences` endpoint
  ...
  - [x] 21.6 Extend `InternalUserController` — add `PUT /internal/users/{userId}/dnd` endpoint
  ...
  - [x] 21.7 Extend `InternalCourtController` — add `GET /internal/courts/{courtId}/reminder-rules` endpoint
  ...
  - [x] 21.8 Create `AvailabilityRedisPubSubPublisher` implementing `AvailabilityPubSubPort`
    ...
    - _Requirements: 6.1, 28.1_
  - [x] 21.9 Create `NoOpAvailabilityPubSubPublisher` for environments without Redis
    - File: `platform-service/.../adapter/out/redis/NoOpAvailabilityPubSubPublisher.java`
    - _Requirements: Design — NoOp pattern_
  - [x] 21.10 Extend `BookingEventKafkaConsumer` in Platform Service
  ...
  - [x] 21.12 Update Platform Service test configurations for new `AvailabilityPubSubPort`
...
- [x] 22. Scheduled jobs — DND flush, cleanup, reminders (Transaction Service)
  - [x] 22.1 Create `DndQueueFlushJob` — Quartz job running every 15 minutes
  ...
  - [x] 22.2 Create `NotificationCleanupJob` — daily Quartz job
  ...
  - [x] 22.3 Create `DeviceTokenCleanupJob` — daily Quartz job
  ...
  - [x] 22.4 Create `BookingReminderJob` and `BookingReminderSchedulingService`
  ...
  - [x] 22.5 Extend `PendingConfirmationReminderJob` — implement notification publishing
  ...
  - [x] 22.6 ~~Extend `BookingCompletionJob`~~ — Verify existing implementation (NO-OP)
    ...
    - _Requirements: 29.2_
  - [x] 22.7 Register new Quartz jobs in `QuartzConfig`
...
- [x] 23. Checkpoint — Ensure all scheduled jobs and Platform Service changes compile
  - Ensure all tests pass, ask the user if questions arise.

- [x] 24. Prometheus metrics and structured logging (Transaction Service + Platform Service)
  - [x] 24.1 Register all notification delivery metrics in Transaction Service
  ...
  - [x] 24.2 Implement structured logging for notification delivery events
  ...
  - [x] 24.3 Register `availability_update_latency_seconds` histogram in Platform Service
    - Measure from Kafka event timestamp to Redis Pub/Sub publish in `BookingEventKafkaConsumer`
    - _Requirements: 6.5_

- [x] 25. Configuration — application.yml and build.gradle.kts updates
  - [x] 25.1 Add Phase 5 configuration to Transaction Service `application.yml`
  ...
  - [x] 25.3 Add Phase 5 configuration to Platform Service `application.yml` (if needed)
    ...
    - _Requirements: 20.4_
  - [x] 25.2 Add Phase 5 dependencies to Transaction Service `build.gradle.kts`
...
- [x] 26. Integration wiring and booking status broadcasting (Transaction Service)
  - [x] 26.1 Wire booking status updates to Redis Pub/Sub for real-time admin portal
    ...
    - _Requirements: 6.6_
  - [x] 26.2 Wire `ScheduleBookingReminderUseCase` into `ConfirmBookingService`
    - On booking confirmation, call `scheduleReminder(bookingId, courtId)`
    - _Requirements: 15.1_
  - [x] 26.3 Wire `CancelBookingReminderUseCase` into `CancelBookingService`
    - On booking cancellation, call `cancelReminder(bookingId)`
    - _Requirements: 15.3_
  - [x] 26.4 Update `GlobalExceptionHandler` in Transaction Service for Phase 5 error responses
    - Handle device token validation errors (400), notification not found (404), encryption errors
    - _Requirements: 2.1, 7.6_
  - [x] 26.5 Update Transaction Service `IntegrationTestConfiguration` for Phase 5 ports
...
- [x] 27. Checkpoint — Full compilation and wiring verification
  - Ensure all tests pass, ask the user if questions arise.

- [x] 28. Unit tests for core services (Transaction Service)
  - [x] 28.1 Write unit tests for `NotificationProcessingService`
  ...
  - [x] 28.3 Write unit tests for `DndConfiguration.isInDndWindow()`
    ...
    - _Requirements: 10.4_
  - [x] 28.4 Write unit tests for `BookingReminderSchedulingService`
    - Test schedule on confirm, cancel on booking cancel
    - Test default 2h reminder when no rule configured
    - Test timezone handling
    - _Requirements: 15.1, 15.3, 15.4, 15.5_
  - [x] 28.5 Write unit tests for `PendingConfirmationReminderJob`
  ...
  - [x] 28.6 Write unit tests for `NotificationCleanupJob`
    - Test 90-day retention cleanup
    - Test 1000-per-user limit enforcement
    - _Requirements: 14.2, 14.5_
  - [x] 28.7 Write unit tests for `DndQueueFlushJob`
    - Test batch of 100, FIFO order
    - Test individual failure handling (no batch rollback)
    - _Requirements: 23.1, 23.2, 23.3_
  - [x] 28.8 Write unit tests for `WebSocketConnectionRegistry`
    - Test register, remove, multiple sessions per user
    - Test concurrent access safety
    - _Requirements: 4.10_
  - [x] 28.9 Write unit tests for `RetryWithBackoff` utility
    ...
    - _Requirements: 3.3, 8.4_
  - [x] 28.10 Write unit tests for `WebSocketBroadcastListener`
...
- [x] 29. Integration tests (Transaction Service + Platform Service)
  - [x] 29.1 Write `NotificationProcessorIntegrationTest`
  ...
  - [x] 29.2 Write `WebSocketIntegrationTest`
  ...
  - [x] 29.5 Write `InternalApiIntegrationTest`
