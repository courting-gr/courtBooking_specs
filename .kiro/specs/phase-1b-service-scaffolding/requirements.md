...
1. THE Common_Library SHALL be a Gradle project with `group` `gr.courtbooking`, `name` `court-booking-common`, and Java 25 source/target compatibility using Kotlin DSL (`build.gradle.kts`).
...
**User Story:** As a developer, I want the Platform Service scaffolded as a Spring Boot 4.x application with hexagonal architecture, so that the codebase is structured for maintainability and testability from the start.

#### Acceptance Criteria

1. THE Platform_Service SHALL be a Gradle project with `group` `gr.courtbooking`, `name` `court-booking-platform-service`, Spring Boot 4.x plugin, and Java 25 source/target compatibility using Kotlin DSL (`build.gradle.kts`).
...
**User Story:** As a developer, I want the Transaction Service scaffolded as a Spring Boot 4.x application with hexagonal architecture, so that the codebase is structured for maintainability and testability from the start.

#### Acceptance Criteria

1. THE Transaction_Service SHALL be a Gradle project with `group` `gr.courtbooking`, `name` `court-booking-transaction-service`, Spring Boot 4.x plugin, and Java 25 source/target compatibility using Kotlin DSL (`build.gradle.kts`).
...
1. THE Platform_Service SHALL include a `Dockerfile` at the repository root that uses a multi-stage build: a Gradle build stage (Java 25 base with gradle:9.3-jdk23) and a runtime stage (Eclipse Temurin 23 JRE slim base).
2. THE Transaction_Service SHALL include a `Dockerfile` at the repository root that uses a multi-stage build: a Gradle build stage (Java 25 base with gradle:9.3-jdk23) and a runtime stage (Eclipse Temurin 23 JRE slim base).
...
2. WHEN a pull request is opened, THE CI_CD_Pipeline SHALL execute: checkout, Java 25 setup, Gradle dependency resolution (with GitHub Packages authentication for Common_Library), compile, unit tests, property-based tests (jqwik), Flyway migration validation (`flyway validate` against a temporary PostgreSQL container), and container image vulnerability scan (Trivy).
...
1. THE Platform_Service and Transaction_Service SHALL use the same Spring Boot plugin version (4.x latest stable) and Java 25 compiler settings.
...
5. THE Common_Library, Platform_Service, and Transaction_Service SHALL all use the same Java 25 source and target compatibility settings via `java.toolchain.languageVersion`.
...
1. THE Platform_Service repository SHALL include a `README.md` at the root with: project overview, prerequisites (Java 25, Gradle 9.3.1, Docker), local setup instructions (Docker Compose + Spring Boot), how to run tests, project structure explanation (hexagonal architecture packages), and links to the API specification and database schema documentation.
